...
INITIAL_ADAPTER_PATH="/home/s2652867/llm-self-play-liquidhaskell/src/Self-Play/SEQ_v2/output/SEQ_deepseek-ai/DeepSeek-R1-Distill-Qwen-7B_SEQ_PROGRAMS500_ITERATIONS7_no_initial_adapter_random_dataset_LR2e-4_EPOCHS3/iteration_7/alice_adapters/checkpoint-537"
TIME="20250807"
NAME="pa_random_dataset_withdiffbiasing_diff5"
N_ITERATIONS=7
LEARNING_RATE=2e-4
NUM_EPOCHS=3
...
do
    ...
    if [ -f "${ITERATION_DIR}/alice_training_data.jsonl" ] && [ -s "${ITERATION_DIR}/alice_training_data.jsonl" ]; then
        ...
        accelerate launch \
            ...
            --previous_adapter_path "$INITIAL_ADAPTER_PATH" \
            --iteration "$i" \
