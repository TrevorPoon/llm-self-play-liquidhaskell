...
class PromptVisitor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        ...
        if self.state != "start":
            print(f"  Setting state to error due to unexpected state: {self.state}")
            self.state = "error"
            return
        ...
        match node.body:
            case [ast.Expr(value=ast.Constant(s)), ast.Pass()] if type(s) == str:
                self.description = s
                self.state = "complete"
                print(f"  FunctionDef visit complete for {node.name}, description set.")
            case _other:
                self.state = "error"
                print(f"  Setting state to error due to unhandled node body pattern for {node.name}: {node.body}")

    def translate_func_decl(self, doctest_transformation: str) -> str | None:
        print(f"Entering translate_func_decl with state: {self.state}")
        if self.state != "complete":
            print(f"Exiting translate_func_decl with None due to incomplete state: {self.state}")
            return None
        ...
        print(f"Exiting translate_func_decl successfully")
...
def translate_prompt(translator, doctest_transformation: str, py_prompt: str, filename: str, added_canonical: str = "") -> str:
    print(f"Entering translate_prompt for {filename}")
    print(f"  doctest_transformation: {doctest_transformation}")
    print(f"  py_prompt: {py_prompt}")
    print(f"filename: {filename}")
    try:
        prompt_ast = ast.parse(py_prompt, filename)
        ...
        print(f"Exiting translate_prompt for {filename} with None due to exception")
        return None
...
def translate_tests(
    ...
    print(f"Entering translate_tests for {filename}")
    ...
    try:
        ...
        traceback.print_exception(e)
        print(f"Exiting translate_tests for {filename} with None due to parsing exception")
        return None
    ...
    for i, item_ast in enumerate(body_ast):
        match item_ast:
            ...
            case _other:
                ...
                print(f"Exiting translate_tests for {filename} with None due to unhandled test case type")
                return None

    if len(test_cases) == len(prefix_lines):
        print("No tests were translated for " + entry_point)
        print(f"Exiting translate_tests for {filename} with None because no tests were translated")
        return None

    for line in translator.test_suite_suffix_lines():
        test_cases.append(line)
    print(f"Exiting translate_tests for {filename} successfully")
    return "\n".join(test_cases)
...
def translate_prompt_and_tests(
    ...
    print(f"Entering translate_prompt_and_tests for {original_file.name}")
    print(f"Translating {original_file.name}")
    ...
    if translated_prompt is None:
        print(f"Skipping {original_file.name} because translated prompt is None")
        print(f"Exiting translate_prompt_and_tests for {original_file.name} with None (translated_prompt is None)")
        return None
    ...
    if translated_tests is None:
        print(f"Skipping {original_file.name} because translated tests are None")
        print(f"Exiting translate_prompt_and_tests for {original_file.name} with None (translated_tests is None)")
        return None

    print(f"Exiting translate_prompt_and_tests for {original_file.name} successfully")
