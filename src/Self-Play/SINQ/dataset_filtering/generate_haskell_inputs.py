    ...
    def load_programs(self, dataset_name):
        ...
        try:
            if os.path.isfile(dataset_name) and dataset_name.endswith('.jsonl'):
                ...
                logger.info(f"Loading programs from JSONL file {dataset_name}...")
                with open(dataset_name, 'r') as f:
                    for line in f:
                        program_item = json.loads(line)
                        if 'code' in program_item and program_item['code']:
                            programs.append(program_item['code'])
            elif os.path.isdir(dataset_name):
                # Load the dataset from the specified directory
                logger.info(f"Loading programs from dataset directory {dataset_name}...")
                ...
                logger.error(f"Invalid dataset_name: {dataset_name}. Must be a .jsonl file or a directory.")
                return []
        except Exception as e:
