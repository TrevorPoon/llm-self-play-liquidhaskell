...
ALICE_USER_PROMPT_TEMPLATE = textwrap.dedent("""
    Original program `P`:
    ```haskell
    {program}
    ```
...
class SInQ:
    ...
    def load_initial_programs(self, dataset_name):
        ...
        try:
            ...
            if os.path.isdir(dataset_name):
                ...
                if self.args.num_initial_programs is None:
            ...
            if self.args.num_initial_programs is not None and len(programs) < self.args.num_initial_programs:
    ...
    def run_alice(self, program_p):
        ...
        messages = [
            ...
            {"role": "user", "content": ALICE_USER_PROMPT_TEMPLATE.format(program=program_p)}
        ]
    ...
    def finetune_model(self, dataset, model_type, iteration):
        ...
        if model_type == "alice" and self.alice_adapter_path and os.path.exists(self.alice_adapter_path):
            print(f"Loading existing adapter weights from {self.alice_adapter_path} for continued training.")
            ...
            print(f"Loading existing adapter weights from {self.bob_adapter_path} for continued training.")
            adapter_path_to_load = self.bob_adapter_path

        if adapter_path_to_load:
            logger.info(f"Loading existing PeftModel from {adapter_path_to_load} for continued training.")
            peft_model = PeftModel.from_pretrained(self.base_model, adapter_path_to_load, is_trainable=True)
        else:
            logger.info("Creating a new PeftModel for initial training.")
    ...
    def run_self_play_loop(self):
        ...
        for i in range(self.args.n_iterations):
            ...
            for j, p_original in enumerate(tqdm(self.programs, desc=f"Iteration {i+1}")):
                ...
                # Filter Alice's training data based on difficulty
                if difficulty >= self.args.difficulty_threshold:
                    logger.info(f"🟩 Example has difficulty {difficulty:.2f} (>= {self.args.difficulty_threshold}). Adding to Alice's training data.")
                    messages = [
                        {"role": "system", "content": ALICE_SYSTEM_PROMPT},
                        {"role": "user", "content": ALICE_USER_PROMPT_TEMPLATE.format(program=p_original)},
                        {"role": "assistant", "content": alice_raw_output}
                    ]
                    alice_training_data.append(self.tokenizer.apply_chat_template(messages, tokenize=False))
                    new_programs.append(q_candidate)
                else:
                    logger.info(f"🟩 Example has difficulty {difficulty:.2f} (< {self.args.difficulty_threshold}). Discarding for Alice's training.")
            ...
            logger.info(f"  - Difficulty threshold: {self.args.difficulty_threshold}")
            logger.info(f"  - Alice training data size: {len(alice_training_data)}")
            ...
            if alice_training_data or bob_training_data:
                ...
                if alice_training_data:
                    self.finetune_model(alice_training_data, "alice", i)
    ...
    def evaluate_agent(self, agent_name, adapter_path, iteration):
        ...
        # The evaluation script now handles loading the base model and applying the adapter.
        # We no longer need to merge the model here.
        ...
        eval_script_path = os.path.join(eval_working_dir, 'eval_script/eval_hs_finetuning.sh')
        ...
        subprocess.run(['sbatch', eval_script_path, adapter_path_abs], check=True, cwd=eval_working_dir)
