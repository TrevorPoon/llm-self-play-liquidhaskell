...
from transformers import  get_linear_schedule_with_warmup, AutoConfig, AutoModelForCausalLM, AutoTokenizer
...
def tokenize(item):
    source, target, max_source_length, max_target_length, tokenizer = item
    
    source_tokens = tokenizer.tokenize(source)[:max_source_length-2]
    source_tokens = [tokenizer.bos_token] + source_tokens + [tokenizer.eos_token]
    source_ids = tokenizer.convert_tokens_to_ids(source_tokens)
    padding_length = max_source_length - len(source_ids)
    source_ids += [tokenizer.pad_token_id] * padding_length

    target_tokens = tokenizer.tokenize(target)[:max_target_length-1]
    target_tokens = target_tokens + [tokenizer.eos_token]
    target_ids = tokenizer.convert_tokens_to_ids(target_tokens)
    padding_length = max_target_length - len(target_ids)
    target_ids += [tokenizer.pad_token_id] * padding_length

    return source_tokens, source_ids, target_ids


def convert_examples_to_features(examples, tokenizer, args, pool=None, stage=None):
    features = []
    
    sources = [(x.source, x.target, args.max_source_length, args.max_target_length, tokenizer) for x in examples]
    if pool is not None:
        tokenize_tokens = pool.map(tokenize, tqdm(sources, total=len(sources)))
    else:
        tokenize_tokens = [tokenize(x) for x in sources]

    for example_index, (source_tokens, source_ids, target_ids) in enumerate(tokenize_tokens):
        if example_index < 5:
            if stage == 'train':
                logger.info("*** Example ***")
...
def main():
    ...
    if args.use_lora:
        logger.info("Using LoRA for parameter-efficient training.")
    ...
    if args.load_model_path is not None:
        ...
        if args.use_lora:
            model = PeftModel.from_pretrained(model, args.load_model_path)
        else:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval:
                ...
                for ref, gold in zip(p, eval_examples):
                    # a bit of a hack to get the predicted part
                    pred = post_process(ref[len(gold.source):].strip())
