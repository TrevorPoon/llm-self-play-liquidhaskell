...
class SEQ:
    def __init__(self, args):
        ...
        self.programs = self.load_programs(args.dataset_name, args.num_initial_programs)
    ...
    def load_programs(self, dataset_name, start_offset: int = 0):

        logger.info(f"Loading initial programs from dataset {dataset_name} with offset {start_offset}...")
        try:
            programs = []
            ...
            if os.path.isdir(dataset_name):
                ...
                if num_to_take==0:
                    ...
                    num_to_take = min(num_to_take, len(dataset) - start_offset)
                    if num_to_take <= 0:
                        logger.warning(f"Warning: No programs to load after offset {start_offset}. Dataset size: {len(dataset)}")
                        return []
                    logger.info(f"Initial programs: {len(dataset)}")
                    logger.info(f"Loading {num_to_take} initial programs starting from offset {start_offset}.")
                    dataset_iterator = dataset.select(range(start_offset, start_offset + num_to_take))
                ...
                if self.args.num_initial_programs==0:
                    ...
                    dataset_iterator = dataset.skip(start_offset)
                else:
                    logger.info(f"Loading {self.args.num_initial_programs} initial programs from stream with offset {start_offset}.")
                    dataset_iterator = dataset.skip(start_offset).take(self.args.num_initial_programs)
            else:
